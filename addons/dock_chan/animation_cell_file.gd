...
static func from_file(file_name: String, directory: String):
	...
	return AnimationCellFile.new(cell_number, animation_name, sheet_name, directory, path)
...
func _init(cell_number: int, animation_name: String, sheet_name: String, directory: String, path: String):
...
func image_path() -> String:
	...
	return self.directory + "/" + self.animation_name + ".png"
